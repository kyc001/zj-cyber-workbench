...
_COMPACTION_LOCKS: dict[CompactionScope, asyncio.Lock] = {}
...
async def compact_if_needed(
    *,
    ...
    if len(candidate_items) < cfg.context_compression_min_items:
        logger.debug(
            "agent context compaction skipped: too few candidate items "
            "session=%s viewer=%s candidates=%d min=%d tokens=%d trigger=%d window=%d",
            scope.session_id,
    ...
    logger.debug(
        "agent context compaction starting session=%s viewer=%s nested_for=%s nested_call=%s "
        "model=%s items=%d candidate_items=%d tokens=%d trigger=%d target=%d window=%d end_message_id=%s",
        scope.session_id,
        scope.viewer_agent_code,
        scope.nested_for,
    ...
    # parents waiting forever). The process lock only needs to guard the fast
    ...
    logger.info(
        "agent context compacted session=%s viewer=%s nested_for=%s nested_call=%s "
        "items=%d tokens=%d summary_tokens=%d window=%d end_message_id=%s",
        scope.session_id,
        scope.viewer_agent_code,
        scope.nested_for,
        scope.nested_call_id,
...
def _select_compaction_candidates(
    ...
    *,
    ...
    if incoming_tokens >= target_tokens:
        logger.debug(
            "incoming prompt tokens exceed compression target: incoming=%d target=%d",
            incoming_tokens,
            target_tokens,
        )
...
async def _try_lock_compaction(sess: AsyncSession, scope: CompactionScope) -> bool:
    del sess
    lock = _COMPACTION_LOCKS.setdefault(scope, asyncio.Lock())
    if lock.locked():
        return False
    await lock.acquire()
    return True
...
async def _unlock_compaction(sess: AsyncSession, scope: CompactionScope) -> None:
    del sess
    lock = _COMPACTION_LOCKS.get(scope)
    if lock is not None and lock.locked():
        lock.release()
    _COMPACTION_LOCKS.pop(scope, None)
