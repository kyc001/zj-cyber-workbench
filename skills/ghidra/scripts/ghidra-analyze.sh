...
# Locate analyzeHeadless from the sandbox image installation.
if [[ -n "${GHIDRA_HOME:-}" && -x "$GHIDRA_HOME/support/analyzeHeadless" ]]; then
    ANALYZE_HEADLESS="$GHIDRA_HOME/support/analyzeHeadless"
elif [[ -x "/usr/local/bin/analyzeHeadless" ]]; then
    ANALYZE_HEADLESS="/usr/local/bin/analyzeHeadless"
    GHIDRA_HOME=$(dirname "$(dirname "$(readlink -f "$ANALYZE_HEADLESS")")")
else
    echo "ERROR: Could not find Ghidra's analyzeHeadless script." >&2
    echo "Expected GHIDRA_HOME/support/analyzeHeadless or /usr/local/bin/analyzeHeadless." >&2
    exit 1
fi
