...
from tests.unit._network_addresses import LOOPBACK_HOST, NON_MATCHING_HOST
...
class NetworkToolTests(unittest.IsolatedAsyncioTestCase):
    def test_ssh_target_host_port_accepts_plain_host_and_port_argument(self) -> None:
        self.assertEqual((LOOPBACK_HOST, 2222), _ssh_target_host_port(LOOPBACK_HOST, 2222))

    def test_ssh_target_host_port_accepts_ssh_url(self) -> None:
        self.assertEqual((LOOPBACK_HOST, 2222), _ssh_target_host_port(f"ssh://{LOOPBACK_HOST}:2222", 22))

    def test_ssh_target_host_port_rejects_non_ssh_scheme(self) -> None:
        with self.assertRaises(ValueError):
            _ssh_target_host_port(f"http://{LOOPBACK_HOST}:2222", 22)

    def test_same_ssh_endpoint_accepts_localhost_alias(self) -> None:
        self.assertTrue(_same_ssh_endpoint("localhost", 2222, LOOPBACK_HOST, 2222))
    ...
    async def test_ssh_context_uses_current_workspace_managed_host(self) -> None:
        ...
        self.assertEqual((LOOPBACK_HOST, 2222, "zj_sandbox", {"password": "secret-value"}, "managed_host"), result)
